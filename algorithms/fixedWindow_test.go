...
func TestFixedWindowConcurrency(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(id int) {
			count := 0
			for j := 0; j < 10; j++ {
				result, err := fw.Allow(ctx, "concurrent_user")
				if err == nil && result.Allowed {
					count++
				}
			}
...
func TestFixedWindowEdgeCase(t *testing.T) {
	...
	result3, err3 := fw.Allow(ctx, "user1")
